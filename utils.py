...
def plot_win_rate(win_history, window_size=100):
    moving_average_rewards = [sum(win_history[i:i+window_size])/window_size for i in range(len(win_history) - window_size + 1)]

    plt.plot(moving_average_rewards)
    plt.xlabel("Episode")
    plt.ylabel("Average win")
    plt.title(f"Average win over {window_size} Episodes")
    plt.show()
